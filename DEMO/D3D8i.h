// D3D v8 Display unit internal header. This is a pretty massive extract from the Direct3D 8.0 application framework.
#ifndef _REV_D3D8I_H_INCLUDED
#define _REV_D3D8I_H_INCLUDED
...
enum D3DAppDef_Notoggle {
	D3DAppDef_FullScreen = 1 << 0,  // Sets default to FULLSCREEN mode.
	D3DAppDef_Notoggle   = 1 << 1,  // Cant toggle between Fullscreen and Windowed mode
	D3DAppDef_Nomenu     = 1 << 2,  // Removes Menu.	
	D3DAppDef_Noaccel    = 1 << 3,  // Removes accelerator table.
	D3DAppDef_ForceDWORD = 0xFFFFFFFF
};

/*class CD3DApplication
{
	LPDIRECT3D8 m_pD3D;
	mword m_dwCreationWidth;
	mword m_dwCreationHeight;
...
protected:
    ...
    DWORD             m_dwDefaultWidth;    // Default Width used to create window (0 means no preferance)
    DWORD             m_dwDefaultHeight;   // Default Height used to create window (0 means no preferance)
	DWORD			  m_dwDefaultBPP;      // Default Bit count per pixel in default fullscreen mode selected
	DWORD			  m_dwDefaultFlags;    // Default setting bits
