...
;	mask tables for tiled textures (4x4 blocks - 2 bits for inner tile position)
	...
	prf16	dd		0.0625, 0.0625, 0.0625, 0.0625
...
align 16
	...
	rf0		dd		0.0,0.0,0.0,0.0
	rf1		dd		1.0,1.0,1.0,1.0
	rf2		dd 		0.5,0.5,0.5,0.5
	rf3		dd		0.333333,0.333333,0.333333,0.333333
	rf4		dd		0.25,0.25,0.25,0.25
	rf5		dd		0.2,0.2,0.2,0.2
	rf6		dd		0.16666667,0.16666667,0.16666667,0.16666667
	rf7		dd		0.142857142857,0.142857142857,0.142857142857,0.142857142857
	rf8		dd		0.125,0.125,0.125,0.125
	rf9		dd		0.11111111,0.11111111,0.11111111,0.11111111
	rf10	dd		0.1,0.1,0.1,0.1
	rf11	dd		0.09090909091,0.09090909091,0.09090909091,0.09090909091
	rf12	dd		0.08333333,0.08333333,0.08333333,0.08333333
	rf13	dd		0.076923,0.076923,0.076923,0.076923
	rf14	dd		0.071429,0.071429,0.071429,0.071429
	rf15	dd		0.06666667,0.06666667,0.06666667,0.06666667
	rf16	dd		0.0625,0.0625,0.0625,0.0625
	rf17	dd		0.0588235,0.0588235,0.0588235,0.0588235
	rf18	dd		0.055556,0.055556,0.055556,0.055556
	rf19	dd		0.052632,0.052632,0.052632,0.052632
...
;;; STATIC VARIABLES ;;;
	...
	public			_IX_TGZSAM_Code_Start
	public			_IX_TGZSAM_Code_Size
	_IX_TGZSAM_Code_Start	dd	ASM_FILE_START
	_IX_TGZSAM_Code_Size	dd	ASM_FILE_END - ASM_FILE_START
...
;;;;;;;;;;;
TGZSAM_CalcRightSection	proc	near
...
; calc integer number of scanlines
	movss	xmm0,	dword ptr [V1 + Vertex_Y]
	movss	xmm1,	dword ptr [V2 + Vertex_Y]
	cvtss2si ebx,	xmm0
	cvtss2si eax,	xmm1
	sub		eax,	ebx
...
; forward loading of vertex X	xmm0 = V1 X
	movss	xmm0,	dword ptr [V1 + Vertex_X]


; Right.dX = (V2->X - V1->X) * rHeight;
; sdword FPRevHeight;

	movss  xmm3,   dword ptr [V2 + Vertex_X]
	subss	xmm3,	xmm0

; forward loading of vertex lighting
	movaps	xmm5,	dword ptr [V1 + Vertex_R]
...
; ebx = ceil(V1->Y)
	cvtsi2ss	xmm1,	ebx
; load V1 X
	subss		xmm1,	dword ptr [V1 + Vertex_Y]
	movss		xmm6,	xmm1
...
; late storing of X and delta
	movss	dword ptr [Right_X],	xmm0
	movss	dword ptr [Right_dX],	xmm3

	movaps	dword ptr [Right_R],	xmm5
	movaps	dword ptr [Right_dR],	xmm4
...
; Returns:		Number of scan lines
;;;;;;;;;;;
TGZSAM_CalcLeftSection		proc	near
...
align 16
	Inner:
		...
		movd	mm5,	dword ptr [ebx*4 + 060000000h]
		org $ - 4
		SMC_Texture dd 0
...
Terminate:
	...
	mov esp,[save_esp]
	ret
TGZSAM_SpanLoop		endp

;;;;;;;;;;;;;;;;
_IX_TGZSAM_AsmFiller	proc	near
...
;	ddx.dRZdx = invArea * ((Verts[1].RZ - Verts[0].RZ) * dy02 - (Verts[2].RZ - Verts[0].RZ) * dy01);
	...
	mov			ebx,		[Arg_Verts]
	; xmm0 = Verts[0].Y
	movss	xmm0,	dword ptr [ebx + Vertex_Y]
	; xmm1 = Verts[1].Y
	movss	xmm1,	dword ptr [ebx + VertexSize + Vertex_Y]
	; xmm2 = Verts[2].Y
	movss	xmm2,	dword ptr [ebx + VertexSize * 2 + Vertex_Y]

	; xmm3 = V[1] - V[0]	xmm4 = V[2] - V[0]	xmm5 = V[0]
	movaps	xmm5,	dword ptr [ebx + Vertex_RZ]
	; xmm1 = dy01
	subss	xmm1,	xmm0
	movaps	xmm3,	dword ptr [ebx + VertexSize + Vertex_RZ]
	shufps	xmm1,	xmm1,	00000000b
	; xmm2 = dy02
	subss	xmm2,	xmm0
